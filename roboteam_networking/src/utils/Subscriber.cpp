...
namespace rtt::net::utils {
    ...
    Subscriber::Subscriber(const ChannelType& channelType, const std::function<void(const std::string& message)> callback)
    ...
    InvalidCallbackException::InvalidCallbackException(const std::string& message)
        : message(message) {
    }
