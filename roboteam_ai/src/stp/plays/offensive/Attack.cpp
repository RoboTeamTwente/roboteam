...
void Attack::calculateInfoForMidfielders() noexcept {
    ...
    if (world->getWorld()->getBall()->get()->position.x > field.getFieldGrid().middleRightCell().left()) {
        ...
        if (world->getWorld()->getBall().value()->position.y > field.getFieldGrid().middleRightCell().top()) {  // Ball is in top of field
