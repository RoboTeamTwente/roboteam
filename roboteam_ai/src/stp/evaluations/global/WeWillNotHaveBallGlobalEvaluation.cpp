...
namespace rtt::ai::stp::evaluation {
    uint8_t WeWillNotHaveBallGlobalEvaluation::metricCheck(const world::World* world, const Field*) const noexcept {
        ...
        return std::any_of(us.begin(), us.end(), [&](auto& robot) { 
            return (LineSegment(ballPosition, endBallPosition).distanceToLine(robot->getPos()) < 0.3 || robot->hasBall()); }) 
            ? stp::control_constants::FUZZY_FALSE : stp::control_constants::FUZZY_TRUE;
    }
}  // namespace rtt::ai::stp::evaluation