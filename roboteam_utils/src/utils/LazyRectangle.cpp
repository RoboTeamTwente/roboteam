...
namespace rtt {
    LazyRectangle::LazyRectangle(const Vector2 &corner, const Vector2 &oppositeCorner)
        : corner1{ corner }, corner2{ oppositeCorner } {
    }
    LazyRectangle::LazyRectangle(const Vector2 &bottomLeft, double x, double y)
    ...
    double LazyRectangle::left() const {
        return fmin(corner1.x, corner2.x);
    }
    double LazyRectangle::right() const {
        return fmax(corner1.x, corner2.x);
    }
    double LazyRectangle::bottom() const {
        return fmin(corner1.y, corner2.y);
    }
    double LazyRectangle::top() const {
        return fmax(corner1.y, corner2.y);
    }
    double LazyRectangle::width() const {
        return fabs(corner1.x - corner2.x);
    }
    double LazyRectangle::height() const {
        return fabs(corner1.y - corner2.y);
    }
    ...
    LineSegment LazyRectangle::topLine() const {
        return { topLeft(), topRight() };
    }
    LineSegment LazyRectangle::rightLine() const {
        return { topRight(), bottomRight() };
    }
    LineSegment LazyRectangle::bottomLine() const {
        return { bottomRight(), bottomLeft() };
    }
    LineSegment LazyRectangle::leftLine() const {
    ...
    Polygon LazyRectangle::asPolygon() const {
        return Polygon(corners());
    }

    Vector2 LazyRectangle::center() const {
        return (corner1 + corner2) * 0.5;
    }

    Vector2 LazyRectangle::topLeft() const {
        return { this->left(), this->top() };
    }
    Vector2 LazyRectangle::topRight() const {
        return { this->right(), this->top() };
    }
    Vector2 LazyRectangle::bottomLeft() const {
        return { this->left(), this->bottom() };
    }
    Vector2 LazyRectangle::bottomRight() const {
    ...
    bool LazyRectangle::doesIntersect(const LineSegment &line) const {
        return !intersects(line).empty();
    }

    // include the boundary for this calculation!
    bool LazyRectangle::contains(const Vector2 &point) const {
    ...
    std::ostream &LazyRectangle::write(std::ostream &os) const {
