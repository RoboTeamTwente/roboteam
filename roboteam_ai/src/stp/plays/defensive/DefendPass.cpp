...
void DefendPass::calculateInfoForOffenders() noexcept {
    stpInfos["offender_1"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, field.getFieldGrid().middleRightCell().getPoints(), gen::OffensivePosition, field, world));
    if (world->getWorld()->getBall().value()->position.y > 0) {
        stpInfos["offender_2"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, field.getFieldGrid().topRightCell().getPoints(), gen::OffensivePosition, field, world));
    } else {
        stpInfos["offender_2"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, field.getFieldGrid().bottomRightCell().getPoints(), gen::OffensivePosition, field, world));
    }
}
