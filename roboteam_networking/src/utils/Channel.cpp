...
namespace rtt::net::utils {
    ...
    Channel::Channel(std::string name, std::string ip, std::string port)
    ...
    std::string Channel::getSubscribeAddress() {
        return getAddress(ip, port);
    }

    std::string Channel::getPublishAddress() {
        return getAddress("*", port);
    }

    std::string Channel::getAddress(const std::string &_ip, const std::string &_port) {
