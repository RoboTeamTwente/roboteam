...
Vector2::Vector2(rtt::Angle &angle, const double &length) : x{cos(angle) * length}, y{sin(angle) * length} {}
...
double Vector2::angle() const { return this->toAngle(); }

rtt::Angle Vector2::toAngle() const { return Angle(atan2(y, x)); }
...
bool Vector2::operator==(const Vector2 &other) const { return fabs(this->x - other.x) < RTT_PRECISION_LIMIT && fabs(this->y - other.y) < RTT_PRECISION_LIMIT; }
