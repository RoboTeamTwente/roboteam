...
[[maybe_unused]] static void hex2bin(const char* src, packed_robot_feedback& target) {
    int i{ 0 }; // don't leave this uninitialized, the value is UB, incrementing it could either be 1, 0, -500, 273613 or whatever else
    /**
     * be consistent
...
[[maybe_unused]] static std::shared_ptr<proto::WorldRobot> getWorldBot(unsigned int id, bool ourTeam, const proto::World &world) {
    ...
    // Should do that instead, but whatever, doesn't really matter in terms of performance
