...
DemoKeeperMove::DemoKeeperMove() { skills = rtt::collections::state_machine<Skill, Status, StpInfo>{skill::GoToPos()}; }

std::optional<StpInfo> DemoKeeperMove::calculateInfoForSkill(StpInfo const &info) noexcept {
    ...
    // auto keepersLineSegment = getKeepersLineSegment(skillStpInfo.getField().value());
    ...
    skillStpInfo.setPositionToMoveTo((targetPosition.first + ballLocation + ballLocation)/3);
...
std::pair<Vector2, PIDType> DemoKeeperMove::calculateTargetPosition(const world::view::BallView &ball, const Field &field,
...
bool DemoKeeperMove::isBallHeadingTowardsOurGoal(const HalfLine &ballTrajectory, const Field &field) {
...
std::optional<HalfLine> DemoKeeperMove::estimateBallTrajectory(const world::view::BallView &ball, const std::optional<world::view::RobotView> &enemyRobot) {
...
bool DemoKeeperMove::shouldTacticReset(const StpInfo &info) noexcept { return true; }

const char *DemoKeeperMove::getName() { return "Demo Keeper Move"; }

LineSegment DemoKeeperMove::getKeepersLineSegment(const Field &field) {
...
Angle DemoKeeperMove::calculateTargetAngle(const world::view::BallView &ball, const Vector2 &targetKeeperPosition) {
