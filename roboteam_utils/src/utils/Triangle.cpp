...
namespace rtt {
    ...
    double Triangle::area() const {
        return fabs((corner1.x * (corner2.y - corner3.y) + corner2.x * (corner3.y - corner1.y) + corner3.x * (corner1.y - corner2.y)) * 0.5);
    }
    std::vector<LineSegment> Triangle::lines() const {
        return std::vector<LineSegment>({ LineSegment(corner1, corner2), LineSegment(corner2, corner3), LineSegment(corner3, corner1) });
    }
    ...
    bool Triangle::doesIntersect(const Line &line) const {
        return line.intersects(LineSegment(corner1, corner2)) || line.intersects(LineSegment(corner2, corner3)) || line.intersects(LineSegment(corner3, corner1));
    }
