...
#include "stp/skills/DemoKeeperDefendGoal.h"
...
DemoKeeperBlockBall::DemoKeeperBlockBall() { skills = rtt::collections::state_machine<Skill, Status, StpInfo>{skill::Rotate(), skill::Rotate(), skill::Rotate(), skill::DemoKeeperDefendGoal()}; }
// DemoKeeperBlockBall::DemoKeeperBlockBall() { skills = rtt::collections::state_machine<Skill, Status, StpInfo>{skill::Rotate(), skill::DemoKeeperDefendGoal()}; }
// DemoKeeperBlockBall::DemoKeeperBlockBall() { skills = rtt::collections::state_machine<Skill, Status, StpInfo>{skill::DemoKeeperDefendGoal()}; }

std::optional<StpInfo> DemoKeeperBlockBall::calculateInfoForSkill(StpInfo const &info) noexcept {
    ...
    if (skills.current_num() == 0) {
        skillStpInfo.setAngle(M_PI*2/3);
    } else if (skills.current_num() == 1) {
        skillStpInfo.setAngle(-M_PI*2/3);
    } else if (skills.current_num() == 2) {
...
bool DemoKeeperBlockBall::isEndTactic() noexcept { return false; }

bool DemoKeeperBlockBall::isTacticFailing(const StpInfo &info) noexcept { return false; }

bool DemoKeeperBlockBall::shouldTacticReset(const StpInfo &info) noexcept { return false; }
...
const char *DemoKeeperBlockBall::getName() { return "Demo Keeper Block Ball"; }

LineSegment DemoKeeperBlockBall::getKeepersLineSegment(const Field &field) {
...
std::optional<HalfLine> DemoKeeperBlockBall::estimateBallTrajectory(const world::view::BallView &ball, const std::optional<world::view::RobotView> &enemyRobot) {
...
bool DemoKeeperBlockBall::isBallHeadingTowardsOurGoal(const HalfLine &ballTrajectory, const Field &field) {
...
std::pair<Vector2, PIDType> DemoKeeperBlockBall::calculateTargetPosition(const world::view::BallView &ball, const Field &field,
...
Angle DemoKeeperBlockBall::calculateTargetAngle(const world::view::BallView &ball, const Vector2 &targetKeeperPosition) {
