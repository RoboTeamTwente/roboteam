...
class static_vector {
   ...
   private:
     ...
     */
    static_assert(std::is_nothrow_copy_constructible_v<element_type> || std::is_nothrow_move_assignable_v<element_type>, "T is not noexcept move or copyable.");

    /**
     * Check whether the contained type is constant, if so we cannot placement-new
     */
    static_assert(!std::is_const_v<element_type>, "T cannot be const.");
   ...
   public:
     ...
     */
    void push_back(const element_type &data) { this->emplace_back(data); }

    /**
     * Push back an element (copy)
     */
    void push_back(element_type &&data) { this->emplace_back(std::move(data)); }
     ...
     */
    element_type &front() noexcept { return _data.front(); }

    /**
     * Reference to the last element
     */
    element_type &back() noexcept { return _data[size() - 1]; }

    /**
     * Pointer to the internal data
     */
    iterator data() noexcept { return &front(); }

    /**
     * Reference to the first element
     */
    const element_type &front() const noexcept { return _data.front(); }

    /**
     * Reference to the last element
     */
    const element_type &back() const noexcept { return _data[size() - 1]; }

    /**
     * Pointer to the internal data
     */
    const_iterator data() const noexcept { return &front(); }

    /**
     * operator[] defined to index the internal array
     */
    element_type &operator[](size_type index) noexcept { return _data[index]; }

    /**
     * Constant version of operator[]
     */
    const element_type &operator[](size_type index) const noexcept { return _data[index]; }
