...
# ================================ GETTERS ================================
    ...
    def get_capacitorCharged(payload):
        return (payload[3] & 0b00001000) > 0;

    @staticmethod
    def get_ballPos(payload):
        _ballPos = ((payload[3] & 0b00000111) << 1) | ((payload[4] & 0b10000000) >> 7);
        return (_ballPos * 0.0666666666666667) + -0.5000000000000000;

    @staticmethod
    def get_rho(payload):
        _rho = ((payload[4] & 0b01111111) << 9) | ((payload[5]) << 1) | ((payload[6] & 0b10000000) >> 7);
        return (_rho * 0.0001220721751736) + 0.0000000000000000;

    @staticmethod
    def get_theta(payload):
        _theta = ((payload[6] & 0b01111111) << 9) | ((payload[7]) << 1) | ((payload[8] & 0b10000000) >> 7);
        return (_theta * 0.0000958752621833) + -3.1415926535897931;

    @staticmethod
    def get_angle(payload):
        _angle = ((payload[8] & 0b01111111) << 9) | ((payload[9]) << 1) | ((payload[10] & 0b10000000) >> 7);
        return (_angle * 0.0000958752621833) + -3.1415926535897931;

    @staticmethod
    def get_wheelLocked(payload):
        return ((payload[10] & 0b01111000) >> 3);

    @staticmethod
    def get_wheelBraking(payload):
        return ((payload[10] & 0b00000111) << 1) | ((payload[11] & 0b10000000) >> 7);

    @staticmethod
    def get_rssi(payload):
        return ((payload[11] & 0b01111111) << 1) | ((payload[12] & 0b10000000) >> 7);
...
# ================================ SETTERS ================================
    ...
    def set_ballSensorSeesBall(payload, ballSensorSeesBall):
        payload[3] = ((ballSensorSeesBall << 5) & 0b00100000) | (payload[3] & 0b11011111);

    @staticmethod
    def set_dribblerSeesBall(payload, dribblerSeesBall):
        payload[3] = ((dribblerSeesBall << 4) & 0b00010000) | (payload[3] & 0b11101111);

    @staticmethod
    def set_capacitorCharged(payload, capacitorCharged):
        payload[3] = ((capacitorCharged << 3) & 0b00001000) | (payload[3] & 0b11110111);

    @staticmethod
    def set_ballPos(payload, ballPos):
        _ballPos = int((ballPos +0.5000000000000000) / 0.0666666666666667);
        payload[3] = ((_ballPos >> 1) & 0b00000111) | (payload[3] & 0b11111000);
        payload[4] = ((_ballPos << 7) & 0b10000000) | (payload[4] & 0b01111111);

    @staticmethod
    def set_rho(payload, rho):
        _rho = int(rho / 0.0001220721751736);
        payload[4] = ((_rho >> 9) & 0b01111111) | (payload[4] & 0b10000000);
        payload[5] = (_rho >> 1);
        payload[6] = ((_rho << 7) & 0b10000000) | (payload[6] & 0b01111111);

    @staticmethod
    def set_theta(payload, theta):
        _theta = int((theta +3.1415926535897931) / 0.0000958752621833);
        payload[6] = ((_theta >> 9) & 0b01111111) | (payload[6] & 0b10000000);
        payload[7] = (_theta >> 1);
        payload[8] = ((_theta << 7) & 0b10000000) | (payload[8] & 0b01111111);

    @staticmethod
    def set_angle(payload, angle):
        _angle = int((angle +3.1415926535897931) / 0.0000958752621833);
        payload[8] = ((_angle >> 9) & 0b01111111) | (payload[8] & 0b10000000);
        payload[9] = (_angle >> 1);
        payload[10] = ((_angle << 7) & 0b10000000) | (payload[10] & 0b01111111);

    @staticmethod
    def set_wheelLocked(payload, wheelLocked):
        payload[10] = ((wheelLocked << 3) & 0b01111000) | (payload[10] & 0b10000111);

    @staticmethod
    def set_wheelBraking(payload, wheelBraking):
        payload[10] = ((wheelBraking >> 1) & 0b00000111) | (payload[10] & 0b11111000);
        payload[11] = ((wheelBraking << 7) & 0b10000000) | (payload[11] & 0b01111111);

    @staticmethod
    def set_rssi(payload, rssi):
        payload[11] = ((rssi >> 1) & 0b01111111) | (payload[11] & 0b10000000);
        payload[12] = ((rssi << 7) & 0b10000000) | (payload[12] & 0b01111111);
