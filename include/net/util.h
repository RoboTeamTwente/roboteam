...
data *alloc_array(data *arr, num &size, num new_size) {
    if ((arr != NULL && new_size == size) || (arr == NULL && new_size == 0)) return (arr);
...
template <class data, class num>
data *alloc_array2(data *arr, num &w, num &h, int new_w, int new_h) MustUseResult;

template <class data, class num>
data *alloc_array2(data *arr, num &w, num &h, int new_w, int new_h) {
    int size = w * h;
