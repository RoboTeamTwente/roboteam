...
static inline bool REM_RobotCommand_get_useCameraAngle(REM_RobotCommandPayload *remrcp){
    return (remrcp->payload[3] & 0b10000000) > 0;
}

static inline float REM_RobotCommand_get_rho(REM_RobotCommandPayload *remrcp){
    uint32_t _rho = ((remrcp->payload[3] & 0b01111111) << 9) | ((remrcp->payload[4]) << 1) | ((remrcp->payload[5] & 0b10000000) >> 7);
...
static inline float REM_RobotCommand_get_theta(REM_RobotCommandPayload *remrcp){
    uint32_t _theta = ((remrcp->payload[5] & 0b01111111) << 9) | ((remrcp->payload[6]) << 1) | ((remrcp->payload[7] & 0b10000000) >> 7);
...
static inline float REM_RobotCommand_get_angle(REM_RobotCommandPayload *remrcp){
    uint32_t _angle = ((remrcp->payload[7] & 0b01111111) << 9) | ((remrcp->payload[8]) << 1) | ((remrcp->payload[9] & 0b10000000) >> 7);
...
static inline float REM_RobotCommand_get_angularVelocity(REM_RobotCommandPayload *remrcp){
    uint32_t _angularVelocity = ((remrcp->payload[9] & 0b01111111) << 9) | ((remrcp->payload[10]) << 1) | ((remrcp->payload[11] & 0b10000000) >> 7);
...
static inline float REM_RobotCommand_get_cameraAngle(REM_RobotCommandPayload *remrcp){
    uint32_t _cameraAngle = ((remrcp->payload[11] & 0b01111111) << 9) | ((remrcp->payload[12]) << 1) | ((remrcp->payload[13] & 0b10000000) >> 7);
...
static inline float REM_RobotCommand_get_dribbler(REM_RobotCommandPayload *remrcp){
    uint32_t _dribbler = ((remrcp->payload[13] & 0b01110000) >> 4);
...
static inline void REM_RobotCommand_set_kickAtAngle(REM_RobotCommandPayload *remrcp, bool kickAtAngle){
    remrcp->payload[2] = ((kickAtAngle << 1) & 0b00000010) | (remrcp->payload[2] & 0b11111101);
}

static inline void REM_RobotCommand_set_doForce(REM_RobotCommandPayload *remrcp, bool doForce){
    remrcp->payload[2] = (doForce & 0b00000001) | (remrcp->payload[2] & 0b11111110);
}

static inline void REM_RobotCommand_set_useCameraAngle(REM_RobotCommandPayload *remrcp, bool useCameraAngle){
    remrcp->payload[3] = ((useCameraAngle << 7) & 0b10000000) | (remrcp->payload[3] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_rho(REM_RobotCommandPayload *remrcp, float rho){
    ...
    remrcp->payload[3] = ((_rho >> 9) & 0b01111111) | (remrcp->payload[3] & 0b10000000);
    remrcp->payload[4] = (_rho >> 1);
    remrcp->payload[5] = ((_rho << 7) & 0b10000000) | (remrcp->payload[5] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_theta(REM_RobotCommandPayload *remrcp, float theta){
    ...
    remrcp->payload[5] = ((_theta >> 9) & 0b01111111) | (remrcp->payload[5] & 0b10000000);
    remrcp->payload[6] = (_theta >> 1);
    remrcp->payload[7] = ((_theta << 7) & 0b10000000) | (remrcp->payload[7] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_angle(REM_RobotCommandPayload *remrcp, float angle){
    ...
    remrcp->payload[7] = ((_angle >> 9) & 0b01111111) | (remrcp->payload[7] & 0b10000000);
    remrcp->payload[8] = (_angle >> 1);
    remrcp->payload[9] = ((_angle << 7) & 0b10000000) | (remrcp->payload[9] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_angularVelocity(REM_RobotCommandPayload *remrcp, float angularVelocity){
    ...
    remrcp->payload[9] = ((_angularVelocity >> 9) & 0b01111111) | (remrcp->payload[9] & 0b10000000);
    remrcp->payload[10] = (_angularVelocity >> 1);
    remrcp->payload[11] = ((_angularVelocity << 7) & 0b10000000) | (remrcp->payload[11] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_cameraAngle(REM_RobotCommandPayload *remrcp, float cameraAngle){
    ...
    remrcp->payload[11] = ((_cameraAngle >> 9) & 0b01111111) | (remrcp->payload[11] & 0b10000000);
    remrcp->payload[12] = (_cameraAngle >> 1);
    remrcp->payload[13] = ((_cameraAngle << 7) & 0b10000000) | (remrcp->payload[13] & 0b01111111);
}
...
static inline void REM_RobotCommand_set_dribbler(REM_RobotCommandPayload *remrcp, float dribbler){
    ...
    remrcp->payload[13] = ((_dribbler << 4) & 0b01110000) | (remrcp->payload[13] & 0b10001111);
}
