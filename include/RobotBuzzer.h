...
static inline float RobotBuzzer_get_duration(RobotBuzzerPayload *rbp){
    uint32_t _duration = ((rbp->payload[3] & 0b00001111) << 12) | ((rbp->payload[4]) << 4) | ((rbp->payload[5] & 0b11110000) >> 4);
...
static inline void RobotBuzzer_set_messageId(RobotBuzzerPayload *rbp, uint32_t messageId){
    rbp->payload[1] = (messageId & 0b00001111) | (rbp->payload[1] & 0b11110000);
}
...
static inline void RobotBuzzer_set_duration(RobotBuzzerPayload *rbp, float duration){
    uint32_t _duration = (uint32_t)(duration / 0.0000762939453125);
    rbp->payload[3] = ((_duration >> 12) & 0b00001111) | (rbp->payload[3] & 0b11110000);
    rbp->payload[4] = (_duration >> 8);
    rbp->payload[5] = ((_duration >> 8) & 0b11110000) | (rbp->payload[5] & 0b00001111);
}
