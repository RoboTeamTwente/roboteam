...
void velocity_Update(){
	if (velState == on){
		...
		translationVelControl(state, velRef, translationalRef);

		float angularRef = angleControl(velRef[body_w], state[body_w]);
...
void velocity_SetRef(float input[3]){
	velRef[body_x] = input[body_x];
	velRef[body_y] = input[body_y];
...
static void translationVelControl(float state[3], float velRef[3], float translationalRef[4]){
	...
	global2Local(velRef, velLocalRef, state[body_w]); //transfer global to local
