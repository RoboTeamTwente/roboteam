...
typedef struct _RobotCommand {
	...
	float rho:16;                // Direction of movement (radians)
	float theta:16;              // Magnitude of movement (m/s)
	float angle:16;              // Absolute angle (rad) / angular velocity (rad/s)
	float cameraAngle:16;        // Angle of the robot as seen by camera (rad)
...
static inline float RobotCommand_get_cameraAngle(RobotCommandPayload *rcp){
    ...
    return (cameraAngle * 6.2831853071795862) + -3.1415926535897931;
}
...
static inline void RobotCommand_set_cameraAngle(RobotCommandPayload *rcp, float _cameraAngle){
    uint16_t cameraAngle = (_cameraAngle - -3.1415926535897931) * 0.1591549430918953;
