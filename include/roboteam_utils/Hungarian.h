...
/// The Dealer sends the cost of moving each robot to each role and receives an assignment vector where the index represents the roles index (equal to the rows column in the cost matrix)
/// This implementation of the Hungarian Assignment Algorithm (aka Munkres) is a slightly modified version of the one introduced in a paper by Robert Pilgrim
