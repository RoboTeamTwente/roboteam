...
void usbutils_enumerate(){
        ...
        for (int i = 0; i < count; i++) {
            ...
            std::cout
                    << "Class: " << usbutils_classToString(desc.bDeviceClass) << " (" << std::to_string(desc.bDeviceClass) << "), "
            ...
            for(int iConfig = 0; iConfig < desc.bNumConfigurations; iConfig++){
                ...
                if(error < 0) std::cout << "│   Error on iConfiguration : " << usbutils_errorToString(error) << std::endl;
