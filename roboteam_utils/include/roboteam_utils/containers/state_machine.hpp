...
class state_machine {
   private:
     ...
     */
    static_assert(type_traits::is_valid_type_v<Base, Enum, SI>,
                  "Type Base does not have the required functions (noexcept): Enum update(SI const&), void initialize(), void terminate()");
   ...
   public:
     ...
     */
    void skip_n(std::size_t n) noexcept {
        current += n;
     ...
     */
    void skip_to(std::size_t n) noexcept {
        current = n;
        current = std::clamp<size_t>(current, 0, _data.size());
    }
     ...
     */
    [[nodiscard]] bool finished() const noexcept { return current_num() == total_count(); }

    /**
     * Gets current index into state machines
     * @return this->current
     */
    [[nodiscard]] size_t current_num() const noexcept { return current; }

    /**
     * Gets total amount of elements in the state machine
     * @return vector.size();
     */
    [[nodiscard]] size_type total_count() const noexcept { return _data.size(); }
     ...
     */
    void initialize() noexcept { get_current()->initialize(); }
     ...
     */
    void reset() noexcept { skip_n(-current_num()); }
};
}  // namespace rtt::collections
