...
const
    ...
    renderRobot = (robot: IWorldRobot, isYellow: boolean) => {
        ...
        drawings.get(robotId)!.update(isUs && uiStore.isaRobotSelected(robotId), isUs, fieldOrientation, robot);
    },
    onPixiTick = () => {
...
watch([() => props.app, () => aiData.state?.gameSettings?.isYellow], () => {
    cleanUp();
    init();
}, {immediate: true});
