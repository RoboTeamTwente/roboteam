...
<template>
  <table class="table w-full">
    ...
    <tr v-for='(cmd, index) in refereeData.data.slice().reverse()' :key="index">
      ...
      <td class="border-b">
        <div class='bg-base-200 rounded-xl border p-1 px-2 font-mono font-bold text-sm w-min flex items-center gap-2 whitespace-nowrap'>
          <font-awesome-icon :icon='sslRefCommandToIconName(cmd.command)' class="w-3 h-3" /> {{ sslRefCommandToString(cmd.command) }}
        </div>
      </td>
      <td class="border-b">
        <div class='bg-base-200 rounded-xl border p-1 px-2 font-mono font-bold text-sm w-min whitespace-nowrap'>
