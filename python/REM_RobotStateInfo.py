...
class REM_RobotStateInfo:
    ...
    dribbleSpeed = 0          # float   [0.000, 20000.000]   dribblerSpeed
    filteredDribbleSpeed = 0  # float   [0.000, 20000.000]   filtered dribblerSpeed
    dribblespeedBeforeGotBall = 0 # float   [0.000, 20000.000]   dribblerSpeed at at the time dribbler thinks it got the ball
...
# ================================ GETTERS ================================
    ...
    def get_dribbleSpeed(payload):
        _dribbleSpeed = ((payload[34] & 0b00001111) << 12) | ((payload[35]) << 4) | ((payload[36] & 0b11110000) >> 4);
        return (_dribbleSpeed * 0.3051804379339285) + 0.0000000000000000;

    @staticmethod
    def get_filteredDribbleSpeed(payload):
        _filteredDribbleSpeed = ((payload[36] & 0b00001111) << 12) | ((payload[37]) << 4) | ((payload[38] & 0b11110000) >> 4);
        return (_filteredDribbleSpeed * 0.3051804379339285) + 0.0000000000000000;

    @staticmethod
    def get_dribblespeedBeforeGotBall(payload):
        _dribblespeedBeforeGotBall = ((payload[38] & 0b00001111) << 12) | ((payload[39]) << 4) | ((payload[40] & 0b11110000) >> 4);
        return (_dribblespeedBeforeGotBall * 0.3051804379339285) + 0.0000000000000000;

    @staticmethod
    def get_bodyXIntegral(payload):
        _bodyXIntegral = ((payload[40] & 0b00001111) << 28) | ((payload[41]) << 20) | ((payload[42]) << 12) | ((payload[43]) << 4) | ((payload[44] & 0b11110000) >> 4);
        return (_bodyXIntegral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_bodyYIntegral(payload):
        _bodyYIntegral = ((payload[44] & 0b00001111) << 28) | ((payload[45]) << 20) | ((payload[46]) << 12) | ((payload[47]) << 4) | ((payload[48] & 0b11110000) >> 4);
        return (_bodyYIntegral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_bodyWIntegral(payload):
        _bodyWIntegral = ((payload[48] & 0b00001111) << 28) | ((payload[49]) << 20) | ((payload[50]) << 12) | ((payload[51]) << 4) | ((payload[52] & 0b11110000) >> 4);
        return (_bodyWIntegral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_bodyYawIntegral(payload):
        _bodyYawIntegral = ((payload[52] & 0b00001111) << 28) | ((payload[53]) << 20) | ((payload[54]) << 12) | ((payload[55]) << 4) | ((payload[56] & 0b11110000) >> 4);
        return (_bodyYawIntegral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_wheel1Integral(payload):
        _wheel1Integral = ((payload[56] & 0b00001111) << 28) | ((payload[57]) << 20) | ((payload[58]) << 12) | ((payload[59]) << 4) | ((payload[60] & 0b11110000) >> 4);
        return (_wheel1Integral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_wheel2Integral(payload):
        _wheel2Integral = ((payload[60] & 0b00001111) << 28) | ((payload[61]) << 20) | ((payload[62]) << 12) | ((payload[63]) << 4) | ((payload[64] & 0b11110000) >> 4);
        return (_wheel2Integral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_wheel3Integral(payload):
        _wheel3Integral = ((payload[64] & 0b00001111) << 28) | ((payload[65]) << 20) | ((payload[66]) << 12) | ((payload[67]) << 4) | ((payload[68] & 0b11110000) >> 4);
        return (_wheel3Integral * 0.0000232830643708) + -50000.0000000000000000;

    @staticmethod
    def get_wheel4Integral(payload):
        _wheel4Integral = ((payload[68] & 0b00001111) << 28) | ((payload[69]) << 20) | ((payload[70]) << 12) | ((payload[71]) << 4) | ((payload[72] & 0b11110000) >> 4);
...
# ================================ SETTERS ================================
    ...
    def set_dribbleSpeed(payload, dribbleSpeed):
        _dribbleSpeed = int(dribbleSpeed / 0.3051804379339285);
        payload[34] = ((_dribbleSpeed >> 12) & 0b00001111) | (payload[34] & 0b11110000);
        payload[35] = (_dribbleSpeed >> 4);
        payload[36] = ((_dribbleSpeed << 4) & 0b11110000) | (payload[36] & 0b00001111);

    @staticmethod
    def set_filteredDribbleSpeed(payload, filteredDribbleSpeed):
        _filteredDribbleSpeed = int(filteredDribbleSpeed / 0.3051804379339285);
        payload[36] = ((_filteredDribbleSpeed >> 12) & 0b00001111) | (payload[36] & 0b11110000);
        payload[37] = (_filteredDribbleSpeed >> 4);
        payload[38] = ((_filteredDribbleSpeed << 4) & 0b11110000) | (payload[38] & 0b00001111);

    @staticmethod
    def set_dribblespeedBeforeGotBall(payload, dribblespeedBeforeGotBall):
        _dribblespeedBeforeGotBall = int(dribblespeedBeforeGotBall / 0.3051804379339285);
        payload[38] = ((_dribblespeedBeforeGotBall >> 12) & 0b00001111) | (payload[38] & 0b11110000);
        payload[39] = (_dribblespeedBeforeGotBall >> 4);
        payload[40] = ((_dribblespeedBeforeGotBall << 4) & 0b11110000) | (payload[40] & 0b00001111);

    @staticmethod
    def set_bodyXIntegral(payload, bodyXIntegral):
        _bodyXIntegral = int((bodyXIntegral +50000.0000000000000000) / 0.0000232830643708);
        payload[40] = ((_bodyXIntegral >> 28) & 0b00001111) | (payload[40] & 0b11110000);
        payload[41] = (_bodyXIntegral >> 20);
        payload[42] = (_bodyXIntegral >> 12);
        payload[43] = (_bodyXIntegral >> 4);
        payload[44] = ((_bodyXIntegral << 4) & 0b11110000) | (payload[44] & 0b00001111);

    @staticmethod
    def set_bodyYIntegral(payload, bodyYIntegral):
        _bodyYIntegral = int((bodyYIntegral +50000.0000000000000000) / 0.0000232830643708);
        payload[44] = ((_bodyYIntegral >> 28) & 0b00001111) | (payload[44] & 0b11110000);
        payload[45] = (_bodyYIntegral >> 20);
        payload[46] = (_bodyYIntegral >> 12);
        payload[47] = (_bodyYIntegral >> 4);
        payload[48] = ((_bodyYIntegral << 4) & 0b11110000) | (payload[48] & 0b00001111);

    @staticmethod
    def set_bodyWIntegral(payload, bodyWIntegral):
        _bodyWIntegral = int((bodyWIntegral +50000.0000000000000000) / 0.0000232830643708);
        payload[48] = ((_bodyWIntegral >> 28) & 0b00001111) | (payload[48] & 0b11110000);
        payload[49] = (_bodyWIntegral >> 20);
        payload[50] = (_bodyWIntegral >> 12);
        payload[51] = (_bodyWIntegral >> 4);
        payload[52] = ((_bodyWIntegral << 4) & 0b11110000) | (payload[52] & 0b00001111);
    ...
    def set_bodyYawIntegral(payload, bodyYawIntegral):
        ...
        payload[52] = ((_bodyYawIntegral >> 28) & 0b00001111) | (payload[52] & 0b11110000);
        payload[53] = (_bodyYawIntegral >> 20);
        payload[54] = (_bodyYawIntegral >> 12);
        payload[55] = (_bodyYawIntegral >> 4);
        payload[56] = ((_bodyYawIntegral << 4) & 0b11110000) | (payload[56] & 0b00001111);
    ...
    def set_wheel1Integral(payload, wheel1Integral):
        ...
        payload[56] = ((_wheel1Integral >> 28) & 0b00001111) | (payload[56] & 0b11110000);
        payload[57] = (_wheel1Integral >> 20);
        payload[58] = (_wheel1Integral >> 12);
        payload[59] = (_wheel1Integral >> 4);
        payload[60] = ((_wheel1Integral << 4) & 0b11110000) | (payload[60] & 0b00001111);
    ...
    def set_wheel2Integral(payload, wheel2Integral):
        ...
        payload[60] = ((_wheel2Integral >> 28) & 0b00001111) | (payload[60] & 0b11110000);
        payload[61] = (_wheel2Integral >> 20);
        payload[62] = (_wheel2Integral >> 12);
        payload[63] = (_wheel2Integral >> 4);
        payload[64] = ((_wheel2Integral << 4) & 0b11110000) | (payload[64] & 0b00001111);
    ...
    def set_wheel3Integral(payload, wheel3Integral):
        ...
        payload[64] = ((_wheel3Integral >> 28) & 0b00001111) | (payload[64] & 0b11110000);
        payload[65] = (_wheel3Integral >> 20);
        payload[66] = (_wheel3Integral >> 12);
        payload[67] = (_wheel3Integral >> 4);
        payload[68] = ((_wheel3Integral << 4) & 0b11110000) | (payload[68] & 0b00001111);
    ...
    def set_wheel4Integral(payload, wheel4Integral):
        ...
        payload[68] = ((_wheel4Integral >> 28) & 0b00001111) | (payload[68] & 0b11110000);
        payload[69] = (_wheel4Integral >> 20);
        payload[70] = (_wheel4Integral >> 12);
        payload[71] = (_wheel4Integral >> 4);
        payload[72] = ((_wheel4Integral << 4) & 0b11110000) | (payload[72] & 0b00001111);
