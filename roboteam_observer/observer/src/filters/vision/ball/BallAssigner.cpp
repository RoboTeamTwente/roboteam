...
BallObservation mergeObservationsByArea(const std::vector<BallObservation>& observations) {
    ...
    double height = 0.0;
    for (const auto& observation : observations) {
        double weight = zero_area ? 1.0 / static_cast<double>(observations.size()) : observation.area / static_cast<double>(totalArea);
...
BallAssignmentResult assign_balls(const std::vector<CameraGroundBallPrediction>& predictions, const std::vector<BallObservation>& observations) {
    ...
    for (const auto& observation : observations) {
        auto closest_prediction =
            std::min_element(assignment.op_pairs.begin(), assignment.op_pairs.end(),
                             [&](const CameraGroundBallPredictionObservationPair& first, const CameraGroundBallPredictionObservationPair& second) {
                                 return (observation.position - first.prediction.position).squaredNorm() < (observation.position - second.prediction.position).squaredNorm();
                             });
        // if there's no close ball or no observations to match it to, we will use it for new filters.
        if (assignment.op_pairs.empty() || (closest_prediction->prediction.position - observation.position).norm() > MAX_ACCEPT_DIST) {
    ...
    for (std::size_t i = 0; i < predictions.size(); ++i) {
        ...
        if (assigned_observations.empty()) {
            ...
            auto best_observation = std::min_element(assigned_observations.begin(), assigned_observations.end(), [prediction](const BallObservation& a, const BallObservation& b) {
            ...
            assignment.op_pairs[i].observation = mergeableObservations.size() > 1 ? mergeObservationsByArea(mergeableObservations) : mergeableObservations.front();
        }
    }
