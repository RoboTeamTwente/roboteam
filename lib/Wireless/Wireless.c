...
SX1280_Settings set = {
        ...
        .txPower = 31, // -18+31=13dBm (max Tx power)
		.packettype = PACKET_TYPE_FLRC,
        .TX_ramp_time = RADIO_RAMP_20_US,
		.periodBase = BASE_62_us,
        .periodBaseCount = 24,
		.syncWords = {0x0, 0x0, 0x0}, // default first setting
        .syncSensitivity = 1, //high sensitivity mode (not syncword)
        .TXoffset = 128,
        .RXoffset = 0,
        .ModParam = {FLRC_BR_1_300_BW_1_2, FLRC_CR_1_0, BT_0_5},
        .PacketParam = {PREAMBLE_LENGTH_16_BITS, FLRC_SYNC_WORD_LEN_P32S, RX_MATCH_SYNC_WORD_3, PACKET_FIXED_LENGTH, RECEIVEPKTLEN, CRC_1_BYTE, NO_WHITENING},
...
SX1280 * Wireless_Init(float channel, SPI_HandleTypeDef * WirelessSpi){
    ...
    setAutoFS(SX,false); // don't go to FS after Tx or Rx
