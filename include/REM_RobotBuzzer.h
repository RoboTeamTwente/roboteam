...
static inline uint32_t REM_RobotBuzzer_get_period(REM_RobotBuzzerPayload *remrbp){
    return ((remrbp->payload[5]) << 4) | ((remrbp->payload[6] & 0b11110000) >> 4);
}

static inline float REM_RobotBuzzer_get_duration(REM_RobotBuzzerPayload *remrbp){
    uint32_t _duration = ((remrbp->payload[6] & 0b00001111) << 12) | ((remrbp->payload[7]) << 4) | ((remrbp->payload[8] & 0b11110000) >> 4);
...
static inline void REM_RobotBuzzer_set_period(REM_RobotBuzzerPayload *remrbp, uint32_t period){
    remrbp->payload[5] = (period >> 4);
    remrbp->payload[6] = ((period << 4) & 0b11110000) | (remrbp->payload[6] & 0b00001111);
}
...
static inline void REM_RobotBuzzer_set_duration(REM_RobotBuzzerPayload *remrbp, float duration){
    ...
    remrbp->payload[6] = ((_duration >> 12) & 0b00001111) | (remrbp->payload[6] & 0b11110000);
    remrbp->payload[7] = (_duration >> 4);
    remrbp->payload[8] = ((_duration << 4) & 0b11110000) | (remrbp->payload[8] & 0b00001111);
}
