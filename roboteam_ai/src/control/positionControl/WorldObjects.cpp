...
void WorldObjects::calculateFieldCollisions(const rtt::Field &field, std::vector<CollisionData> &collisionDatas, const std::vector<Vector2> &pathPoints, double timeStep,
    ...
    for (size_t i = completedTimeSteps; i < pathPoints.size(); i++) {
        if (!field.playArea.contains(pathPoints[i], rtt::ai::stp::control_constants::OUT_OF_FIELD_MARGIN)) {
            ...
            insertCollisionData(collisionDatas, CollisionData{pathPoints[i], timeStep * (i - completedTimeSteps), rtt::ai::control::CollisionType::FIELD});
            return;
        }
    }
}
...
void WorldObjects::calculateDefenseAreaCollisions(const rtt::Field &field, std::vector<CollisionData> &collisionDatas, const std::vector<Vector2> &pathPoints, double timeStep,
    ...
    for (size_t i = completedTimeSteps; i < pathPoints.size(); i++) {
        if (ourDefenseArea.contains(pathPoints[i]) || theirDefenseArea.contains(pathPoints[i])) {
            ...
            insertCollisionData(collisionDatas, CollisionData{pathPoints[i], timeStep * (i - completedTimeSteps), rtt::ai::control::CollisionType::DEFENSEAREA});
            return;
        }
    }
}
...
void WorldObjects::calculateBallCollisions(const rtt::world::World *world, std::vector<CollisionData> &collisionDatas, const std::vector<Vector2> &pathPoints, double timeStep,
    ...
    for (int i = 0; i <= numSteps; ++i, completedTimeSteps++) {
        ...
        if (distance < dist) {
            insertCollisionData(collisionDatas, CollisionData{pathPoints[completedTimeSteps], loopTime, rtt::ai::control::CollisionType::BALL});
            return;
        }
    }
}
    ...
    for (int i = 0; i <= numSteps; ++i, completedTimeSteps++) {
        ...
        for (const auto &opponentRobot : theirRobots) {
            ...
            if (distance < avoidanceDistance) {
                insertCollisionData(collisionDatas, CollisionData{ourLocationAtMinTime, loopTime, rtt::ai::control::CollisionType::ENEMYROBOT});
                return;
            }
        }
    }
    ...
    for (int i = 0; i <= numSteps; ++i, completedTimeSteps[robotId]++) {
        ...
        for (const auto &ourRobot : ourRobots) {
            ...
            if (distanceSquared < avoidanceDistance) {
                insertCollisionData(collisionDatas, CollisionData{ourLocationAtMinTime, loopTime, rtt::ai::control::CollisionType::OWNROBOT});
                return;
            }
        }
    }
...
void WorldObjects::calculateBallPlacementCollision(const rtt::world::World *world, std::vector<CollisionData> &collisionDatas, const std::vector<Vector2> &pathPoints,
    ...
    for (size_t i = completedTimeSteps; i < pathPoints.size(); i++) {
        if (ballPlacementLine.distanceToLine(pathPoints[i]) < ai::stp::control_constants::AVOID_BALL_DISTANCE) {
            insertCollisionData(collisionDatas, CollisionData{pathPoints[i], timeStep * (i - completedTimeSteps), rtt::ai::control::CollisionType::BALLPLACEMENT});
            return;
        }
    }
}
