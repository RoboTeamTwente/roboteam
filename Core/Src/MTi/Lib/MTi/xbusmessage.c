...
static uint16_t messageLength(struct XbusMessage const* message)
{
	switch (message->mid)
	{
		case XMID_SetOutputConfiguration:
...
// made by leon, not working correctly
uint16_t XbusMessage_getOutputFreq(enum XsDataIdentifier id, struct XbusMessage const* message){
	uint16_t result = 0;
	uint8_t const* raw = getPointerToData(id, message->data, message->length*2*sizeof(uint16_t));
	if (raw)
	{
		switch (id)
		{
			case XDI_Temperature:
				raw = XbusUtility_readU16(&result, raw);
				break;
			case XDI_UtcTime:
				raw = XbusUtility_readU16(&result, raw);
				break;
