...
void AttackingPass::calculateInfoForRoles() noexcept {
    ...
    if (!ballKicked()) {
        ...
        // Passer now goes to a right grid, where the receiver is not
        if (receiverLocation.y > field.getFieldGrid().topRightCell().bottom()) {  // Receiver is going to top of the field
            stpInfos["passer"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, field.getFieldGrid().bottomMiddleCell().getPoints(), gen::BlockingPosition, field, world));
        } else if (receiverLocation.y < field.getFieldGrid().middleMiddleCell().bottom()) {  // Receiver is going to bottom of the field
            stpInfos["passer"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, field.getFieldGrid().topMiddleCell().getPoints(), gen::BlockingPosition, field, world));
        } else {  // Receiver is going to middle of the field, passer will go to the closest grid on the side of the field
            auto targetGrid = stpInfos["passer"].getRobot()->get()->getPos().y < 0 ? field.getFieldGrid().bottomMiddleCell() : field.getFieldGrid().topMiddleCell();
            stpInfos["passer"].setPositionToMoveTo(PositionComputations::getPosition(std::nullopt, targetGrid.getPoints(), gen::BlockingPosition, field, world));
        }
    }
...
void AttackingPass::calculateInfoForAttackers() noexcept {
    if (passInfo.passLocation.y > field.getFieldGrid().topRightCell().bottom()) {  // Receiver is going to left of the field
    ...
    } else if (passInfo.passLocation.y < field.getFieldGrid().middleMiddleCell().bottom()) {  // Receiver is going to right of the field
