...
class circular_buffer {
   ...
   public:
    ...
    void push_back(const Tp &value) { emplace_back(value); }

    void push_back(Tp &&value) { emplace_back(std::move(value)); }
    ...
    // Element access
    constexpr reference operator[](size_type n) noexcept { return _m_buff[(_current_offset + n) % Size]; }

    constexpr const_reference operator[](size_type n) const noexcept { return _m_buff[(_current_offset + n) % Size]; }
    ...
    constexpr const_reference back() const noexcept { return _m_buff[(_current_offset + _current_size - 1) % Size]; }
};
}  // namespace rtt::collections
