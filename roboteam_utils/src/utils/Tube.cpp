...
namespace rtt {
    ...
    Tube::Tube(const LineSegment& line, double radius)
        : lineSegment{ line }, radius{ radius } {
    }
    Tube::Tube(const Vector2& start, const Vector2& end, double radius)
        : lineSegment{ LineSegment(start, end) }, radius{ radius } {
    }
    Tube::Tube()
        : lineSegment{ { 0, 0 }, { 0, 0 } }, radius{ 1.0 } {  // lines default construct to (0,0) to (0,0)
    }
    ...
    Vector2 Tube::project(const Vector2& point) const {
